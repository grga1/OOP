...
protected:
    float osnovna_cena;
    char pretplatnicki_broj[16];
    static float ddv;
    void copy_sms(const SMS &other){
        this->osnovna_cena=other.osnovna_cena;
        strcpy(this->pretplatnicki_broj,other.pretplatnicki_broj);
    }
public:
    SMS(){
        this->osnovna_cena=0;
        strcpy(this->pretplatnicki_broj,"");
    }
    SMS(float cena,char *broj){
        this->osnovna_cena=cena;
        strcpy(this->pretplatnicki_broj,broj);
    }
    SMS(const SMS &other){
        copy_sms(other);
    }
    ...
    friend ostream& operator<<(ostream& out, SMS& sms) {
        out << "Tel: " << sms.pretplatnicki_broj << " - cena: " << sms.SMS_cena() << "den.\n";
        return out;
    }
    ~SMS(){}
};
...
public:
    RegularSMS(){
        strcpy(this->poraka,"msg");
        this->roaming=false;
    }
    RegularSMS(char *tel,float cena,char *msg, bool roam):SMS(cena,tel){
        strcpy(this->poraka,msg);
        this->roaming=roam;
    }
    float SMS_cena(){
  ...
  if(roaming==1)
      ...
      x=ddv;
  return  osnovna_cena*(1.0+x)*ceil(strlen(poraka)/160.0);
    }
...
public:
    SpecialSMS(char *tel,float cena,bool hum):SMS(cena,tel){
        this->humanitarni=hum;
    }
